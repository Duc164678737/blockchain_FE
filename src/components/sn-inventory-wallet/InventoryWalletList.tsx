...
import { QueryInventoryProps } from "redux-store/inventory.redux";
...
const InventoryWalletList = ({ typeItemClass, pathname, NftComponent }: TypeNftComponent) => {
  ...
  const { pageData } = useSelector(InventorySelector.getNftList, shallowEqual);
  const inventoryQuery = useSelector(InventorySelector.getQueryInventory, shallowEqual);

  const handleGetNftList = (query: QueryInventoryProps, ownerAddress: string) => {
    dispatch(
      InventoryActions.getNftInventory({
        filters: {
          ...query,
  ...
  useEffect(() => {
    if (walletAddress) {
      handleGetNftList(inventoryQuery, walletAddress);
    }
  }, [inventoryQuery, walletAddress]);
