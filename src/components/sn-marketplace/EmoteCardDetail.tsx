...
const EmoteCardDetail = ({ data, className, buttonProps, ...otherProps }: EmoteCardProps) => {
    ...
    <Box className={clsx(classes.root, className)}>
      <Stack className={clsx(classes.container)} {...otherProps}>
        ...
        <AppTypography className={classes.price}>
          <AppTrans i18nKey={getLabel("fmPriceEmote", { total: data.price })} />
        </AppTypography>
      </Stack>
...
type EmoteCardProps = StackProps & {
  data: NFTClass;
  buttonProps?: AppButtonProps;
};

export default memo(EmoteCardDetail);
